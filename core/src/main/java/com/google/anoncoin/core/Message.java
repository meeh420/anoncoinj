...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Anoncoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion Anoncoin protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = anoncoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Anoncoin protocol.
    ...
    /**
     * Should only used by AnoncoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by AnoncoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeAnoncoinSerialize()}, which is safe to mutate.
    ...
    public byte[] anoncoinSerialize() {
        byte[] bytes = unsafeAnoncoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the anoncoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeAnoncoinSerialize() {
        ...
        try {
            anoncoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to anoncoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the anoncoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void anoncoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use anoncoinSerialize().
     */
    void anoncoinSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented anoncoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so AnoncoinSerializer can avoid 2 instanceof checks + a casting.
     *
     * @return
     */
