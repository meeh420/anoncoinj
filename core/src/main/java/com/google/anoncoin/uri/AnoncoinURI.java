...
/**
 * <p>Provides a standard implementation of a Anoncoin URI with support for the
 * following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code anoncoin:<address>}</li>
 * <li>{@code anoncoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://en.anoncoin.it/wiki/BIP_0021">BIP 0021</a>
 */
public class AnoncoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(AnoncoinURI.class);
    ...
    public static final String ANONCOIN_SCHEME = "anoncoin";
    ...
    /**
     * Constructs a new AnoncoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws AnoncoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public AnoncoinURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws AnoncoinURIParseException If the input fails Anoncoin URI syntax and semantic checks.
     */
    public AnoncoinURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new AnoncoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  anoncoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form anoncoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the anoncoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : anoncoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("anoncoin://")) {
            schemeSpecificPart = input.substring("anoncoin://".length());
        } else if (input.startsWith("anoncoin:")) {
            schemeSpecificPart = input.substring("anoncoin:".length());
        } else {
            throw new AnoncoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new AnoncoinURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new AnoncoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a Anoncoin address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new AnoncoinURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new AnoncoinURIParseException("Malformed Anoncoin URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new AnoncoinURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The Anoncoin Address from the URI
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("AnoncoinURI[");
        boolean first = true;
    ...
    public static String convertToAnoncoinURI(Address address, BigInteger amount, String label, String message) {
        return convertToAnoncoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Anoncoin URI builder using known good fields.
     * 
     * @param address The Anoncoin address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Anoncoin URI
     */
    public static String convertToAnoncoinURI(String address, BigInteger amount, String label, String message) {
        ...
        builder.append(ANONCOIN_SCHEME).append(":").append(address);
        
        boolean questionMarkHasBeenOutput = false;
        
        if (amount != null) {
            builder.append(QUESTION_MARK_SEPARATOR).append(FIELD_AMOUNT).append("=");
            builder.append(Utils.anoncoinValueToPlainString(amount));
