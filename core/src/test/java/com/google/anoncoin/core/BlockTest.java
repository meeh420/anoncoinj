...
public class BlockTest {
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = new Block(params, header.anoncoinSerialize());
        assertEquals(reparsed, header);
    }

    @Test
    public void testAnoncoinSerialization() throws Exception {
    ...
    public void testJavaSerialiazation() throws Exception {
        ...
        // Note that this will actually check the transactions are equal by doing anoncoin serialization and checking
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.anoncoinSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.anoncoinSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.anoncoinSerialize().length, tx.length);
        ...
        assertEquals(block.length, block.anoncoinSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
